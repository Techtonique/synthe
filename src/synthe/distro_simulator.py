...
class DistroSimulator:
    ...
    def _kl_divergence(self, p_samples, q_samples, epsilon=1e-10):
        """
        Compute KL divergence KL(P||Q) between two distributions.

        P is the observed distribution, Q is the synthetic distribution.
        KL(P||Q) = ∫ p(x) * log(p(x)/q(x)) dx

        Parameters:
        -----------
        p_samples : array-like
            Samples from the observed distribution P
        q_samples : array-like
            Samples from the synthetic distribution Q
        ...
        # Use KDE to estimate the densities
        # Use Scott's rule for bandwidth selection
        from sklearn.neighbors import KernelDensity

        # Fit KDE on P (observed) and Q (synthetic)
        p_kde = KernelDensity(kernel="gaussian", bandwidth="scott")
        q_kde = KernelDensity(kernel="gaussian", bandwidth="scott")

        p_kde.fit(p_samples)
        q_kde.fit(q_samples)

        # Evaluate log densities at P samples
        log_p = p_kde.score_samples(p_samples)  # log p(x)
        log_q = q_kde.score_samples(p_samples)  # log q(x)

        # KL(P||Q) = E_P[log(p(x)/q(x))] = E_P[log p(x) - log q(x)]
        # Monte Carlo estimate using samples from P
    ...
    def fit(self, Y, n_train=None, metric="energy", n_trials=50, **kwargs):
        ...
        metric : str, default='energy'
            Distance metric for optimization ('energy', 'mmd', 'kl', or 'wasserstein')
        ...
        **kwargs : dict
        ...
        def objective(trial):
            ...
            if metric == "energy":
                ...
                dist_val = self._kl_divergence(Y_test, Y_sim)
            elif metric == "wasserstein" and d == 1:
                dist_val = stats.wasserstein_distance(
                    Y_test.flatten(), Y_sim.flatten()
                )
            else:
                raise ValueError(f"Invalid metric '{metric}' for dimension {d}")

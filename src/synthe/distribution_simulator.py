...
try:
    ...
    HAS_OPTUNA = False
    warnings.warn(
        "Optuna not installed. Using grid search for hyperparameters."
    )
...
class DistributionSimulator:
    ...
    def __init__(
        ...
        if model_type not in ["krr", "xgboost"]:
            raise ValueError(
                f"model_type must be 'krr' or 'xgboost', got '{model_type}'"
            )
    ...
    def _compute_mmd(
        self, u: np.ndarray, v: np.ndarray, sigma: float = 1.0
    ) -> float:
    ...
    def _compute_energy_distance(self, u: np.ndarray, v: np.ndarray) -> float:
    ...
    def _compute_metric(self, Y_true: np.ndarray, Y_sim: np.ndarray) -> float:
        ...
        if metric == "wasserstein" and d == 1:
            return stats.wasserstein_distance(Y_true.flatten(), Y_sim.flatten())
        elif metric == "mmd":
    ...
    def _generate_pseudo(
        self, model, residuals: np.ndarray, n_samples: int
    ) -> np.ndarray:
    ...
    def fit(self, Y: np.ndarray) -> "DistributionSimulator":
        ...
        if d >= 2 and self.use_copula:
            ...
            try:
                ...
                warnings.warn(
                    f"Copula fitting failed: {e}. Falling back to surrogate method."
                )
        ...
        if self.verbose:
            print(
                f"Fitting {d}D distribution with {len(Y_train)} training samples"
            )
        ...
        if HAS_OPTUNA:
            ...
            study.optimize(
                lambda trial: self._objective_optuna(
                    trial, Y_train, Y_test, X_train
                ),
            ...
            if self.model_type != "krr":
                raise ValueError(
                    "Grid search only supports KRR. Install Optuna for XGBoost."
                )
    ...
    def sample(self, n_samples: int = 100) -> np.ndarray:
        ...
        if not self.is_fitted_:
            raise ValueError(
                "Simulator must be fitted before sampling. Call fit() first."
            )

        # Use copula if fitted
        if self.copula_ is not None:
            samples = self.copula_.sample(
                n_samples=n_samples,
                method="gmm",
                random_state=self.random_state,
            )
    ...
    def __repr__(self) -> str:
        if self.is_fitted_:
            method = (
                "Copula"
                if self.copula_ is not None
                else self.model_type.upper()
            )
            return (
                f"DistributionSimulator(method='{method}', "
    ...
    def __repr__(self) -> str:
        if self.is_fitted_:
            return (
                f"DistributionSimulator(model_type='{self.model_type}', "
                f"n_dims={self.n_dims_}, fitted=True)"
            )
        return f"DistributionSimulator(model_type='{self.model_type}', fitted=False)"

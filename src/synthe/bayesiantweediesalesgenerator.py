...
class BayesianTweedieSalesGenerator:
    ...
    def _initialize_parameters(self):
        ...
        self.params = {
            ...
            'power_prior': (1.5, 0.2),  # (mean, std) for normal prior on power parameter
            'alpha_prior': (1.0, 0.5),  # (mean, std) for normal prior on regularization
    ...
    def sample_tweedie_parameters(self, n_samples=1000, prior_type='informative'):
        """
        ...
        if prior_type == 'informative':
            ...
            power_samples = np.random.normal(1.6, 0.2, n_samples)  # Compound Poisson-Gamma
            alpha_samples = np.random.gamma(2, 0.5, n_samples)     # Regularization
            dispersion_samples = np.random.gamma(3, 0.3, n_samples) # Dispersion
    ...
    def fit_tweedie_generative_model(self, X, y, n_models=10):
        ...
        """
        print(f"Fitting {n_models} TweedieRegressor models for generative sampling...")
    ...
    def sample_from_tweedie_posterior(self, X, n_samples=1000, model_weights=None):
        """
        ...
        if not self.tweedie_models:
            raise ValueError("No models fitted. Call fit_tweedie_generative_model first.")
        ...
        return pd.DataFrame(np.array(posterior_samples).T, columns=[f'sample_{i}' for i in range(n_samples)])
    ...
    def _sample_tweedie_distribution(self, mu, power, dispersion):
        ...
        for mean_val in mu:
            if power == 1:  # Poisson
                ...
                lambda_val = (mean_val ** (2 - power)) / ((2 - power) * dispersion)
                phi = dispersion
                p = (power - 1) / (2 - power)
                ...
                if N > 0:
                    ...
                    claim_size = np.random.gamma(-p * N, phi * (mean_val ** (power - 1)))
                    sample = claim_size
                else:
                    sample = 0
    ...
    def generate_sales_timeseries(self, product_id, store_id, state, n_days=730):
        """
        ...
        # Generate dates
        dates = pd.date_range(start=self.params['start_date'], periods=n_days, freq='D')
        ...
        if self.tweedie_models:
            ...
            feature_cols = [col for col in covariates.columns if col not in ['date', 'product_id', 'store_id', 'state']]
            X_features = covariates[feature_cols]
            
            # Generate posterior predictive samples
            posterior_samples = self.sample_from_tweedie_posterior(X_features, n_samples=100)
            ...
            # Store samples
            for i in range(min(10, posterior_samples.shape[1])):  # Store first 10 samples
    ...
    def generate_complete_dataset(self, use_bayesian=True, n_samples_per_series=100):
        """
        ...
        for store_id in range(1, self.params['num_stores'] + 1):
            store_state_mapping[store_id] = np.random.choice(self.hierarchies['states'])

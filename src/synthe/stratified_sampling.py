...
class StratifiedClusteringSubsampling:
    def __init__(self, n_components=3, method=ClusterMethod.GMM, random_state=None, **kwargs):
        """
        ...
        self.method = method if isinstance(method, ClusterMethod) else ClusterMethod(method.lower())
        ...
        if self.method == ClusterMethod.GMM:
            ...
            raise ValueError(f"Unsupported method: {method}. Choose 'gmm' or 'kmeans'.")
    ...
    def fit(self, data):
        ...
        if len(data) < self.n_components:
            raise ValueError(f"Number of samples ({len(data)}) must be >= n_components ({self.n_components})")
    ...
    def stratified_sample(self, data, test_size=0.3):
        ...
        if len(data) != len(self.cluster_labels):
            raise ValueError("Data length must match fitted cluster labels length")

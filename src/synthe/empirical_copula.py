...
class EmpiricalCopula:
    ...
    def fit(self, X: np.ndarray) -> 'EmpiricalCopula':
        ...
        if self.smoothing_method != "none":
            self.pseudo_observations_ = self._apply_smoothing(self.pseudo_observations_)        
    ...
    def sample(self, n_samples: int = 50, 
        ...
        if not self.is_fitted_:
            raise ValueError("Copula must be fitted before sampling. Call fit() first.")
    ...
    def validate_fit(self, X_test: Optional[np.ndarray] = None, n_bootstrap: int = 1000,
        ...
        for j in range(self.n_vars_):
            ...
            ks_stat, ks_pvalue = stats.ks_2samp(original_margin, 
                                                simulated_margin)            
            ...
            try:
                ...
                combined_data = np.concatenate([original_margin, 
                                                simulated_margin])
                ...
                ad_critical = ad_orig.critical_values[2]  # 5% significance level
                ad_pass = ad_diff < ad_critical * 0.5  # Heuristic threshold                
            except:
                ad_diff = np.nan
                ad_pass = None            
            # Two-sample t-test for means
            ttest_stat, ttest_pvalue = stats.ttest_ind(original_margin, 
                                                       simulated_margin)            
            # Levene's test for equal variances
            levene_stat, levene_pvalue = stats.levene(original_margin, 
                                                      simulated_margin)
            ...
            if verbose:
                ...
                print(f"  KS test: statistic={ks_stat:.4f}, p-value={ks_pvalue:.4f} [{status}]")
                print(f"  Mean test: p-value={ttest_pvalue:.4f} [{mean_status}]")
                print(f"  Variance test: p-value={levene_pvalue:.4f} [{var_status}]")
        ...
        if verbose:
            for pair, tests in dependence_results.items():
                corr_status = "FAIL" if tests['pearson_significant_diff'] else "PASS"
                spear_status = "WARN" if tests['spearman_large_diff'] else "PASS"
                kendall_status = "WARN" if tests['kendall_large_diff'] else "PASS"
        ...
        for j in range(self.n_vars_):
            ...
            def cvm_uniform(data):
                ...
                i = np.arange(1, n + 1)
                T = (1.0 / (12 * n)) + np.sum(((2 * i - 1) / (2 * n) - sorted_data) ** 2)
                return T
            
            cvm_stat = cvm_uniform(pseudo_margin)
            # Critical value at 5% significance level
            cvm_critical = 0.461 / (np.sqrt(len(pseudo_margin)) + 0.25 + 0.75/np.sqrt(len(pseudo_margin)))
            ...
            if verbose:
                ...
                ad_status = "FAIL" if ad_result.statistic > ad_result.critical_values[2] else "PASS"
                cvm_status = "FAIL" if cvm_stat > cvm_critical else "PASS"
                print(f"Variable {j+1}:")
                print(f"  KS uniform: stat={ks_uniform_stat:.4f}, p-val={ks_uniform_pvalue:.4f} [{ks_status}]")
                print(f"  AD normal: stat={ad_result.statistic:.4f}, crit={ad_result.critical_values[2]:.4f} [{ad_status}]")
                print(f"  CvM uniform: stat={cvm_stat:.4f}, crit={cvm_critical:.4f} [{cvm_status}]")
        ...
        mean_failures = sum(1 for j in range(self.n_vars_) 
                           if results['marginal_tests'][f'variable_{j+1}']['mean_difference_significant'])
        
        var_failures = sum(1 for j in range(self.n_vars_) 
                          if results['marginal_tests'][f'variable_{j+1}']['variance_difference_significant'])
        
        corr_failures = sum(1 for tests in dependence_results.values() 
        ...
        if verbose:
            ...
            print(f"  Variance difference failures: {var_failures}/{self.n_vars_}")
            print(f"  Correlation failures: {corr_failures}/{len(dependence_results)}")
    ...
    def _apply_smoothing(self, pseudo_obs: np.ndarray) -> np.ndarray:
        ...
        if self.smoothing_method == "jitter":
            # Add uniform jitter
            jitter = np.random.uniform(-self.jitter_scale/2, 
                                       self.jitter_scale/2, 
                                       pseudo_obs.shape)
    ...
    def _fit_marginal_transforms(self, X: np.ndarray) -> None:
        ...
        for j in range(self.n_vars_):
            ...
            extended_data = np.concatenate([[sorted_data[0] - (sorted_data[1] - sorted_data[0])],
                                          sorted_data,
                                          [sorted_data[-1] + (sorted_data[-1] - sorted_data[-2])]])            
            quantile_func = interp1d(extended_probs, extended_data, 
    ...
    def _kde_sample(self, n_samples: int, kernel="gaussian", **kwargs) -> np.ndarray:
        """Generate samples using kernel density estimation."""
        kernel_bandwidths = {"bandwidth": np.logspace(-6, 6, 150)}
        grid = GridSearchCV(KernelDensity(kernel=kernel, **kwargs),
    ...
    def __repr__(self) -> str:
        if self.is_fitted_:
            return (f"EmpiricalCopula(n_samples={self.n_samples_}, n_vars={self.n_vars_}, "

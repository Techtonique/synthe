...
class MaximumEntropyBootstrap:
    ...
    def _calculate_trimmed_mean_diff(self, x: np.ndarray) -> float:
        ...
        return np.mean(trimmed_diffs) if len(trimmed_diffs) > 0 else np.mean(diffs)
    ...
    def fit(self, x: Union[np.ndarray, List, pd.Series]) -> 'MaximumEntropyBootstrap':
        ...
        self.intermediate_points_ = (self.order_stats_[:-1] + self.order_stats_[1:]) / 2
    ...
    def _generate_me_quantiles(self, size: int) -> np.ndarray:
        ...
        for i in range(size):
            ...
            quantiles[i] = interval_start + interval_frac * (interval_end - interval_start)
    ...
    def plot_me_density(self, figsize: Tuple[int, int] = (12, 8)) -> plt.Figure:
        ...
        for i in range(len(all_z_points) - 1):
            ax1.axvspan(all_z_points[i], all_z_points[i+1], alpha=0.3, 
        ...
        ax2.plot(self.original_series_, 'o-', label='Original Series', alpha=0.7)
    ...
    def plot_bootstrap_ensemble(self, reps: int = 50, figsize: Tuple[int, int] = (15, 10)) -> plt.Figure:
        ...
        ax1.plot(time_index, self.original_series_, 'r-', linewidth=2, label='Original')
        ...
        ax2.fill_between(time_index, ci_lower, ci_upper, alpha=0.3, label='95% CI')
        ...
        if len(time_index) >= 5:
            ...
            for i, t in enumerate(selected_times):
                ax3.hist(ensemble[t, :], bins=30, alpha=0.5, 
    ...
    def plot_sampling_distribution(self, statistic: Callable, reps: int = 999, 
        ...
        ax1.axvline(original_stat, color='red', linestyle='--', linewidth=2, 
                   label=f'Original: {original_stat:.3f}')
        ax1.axvline(np.mean(stats_boot), color='green', linestyle='--', linewidth=2,
    ...
    def hypothesis_test(self, statistic: Callable, null_value: float = 0,
                       alternative: str = 'two-sided', reps: int = 999,
                       confidence: float = 0.95) -> HypothesisTestResult:
        """
        ...
        if alternative == 'two-sided':
            ...
            ci_upper = np.percentile(stats_boot, (1 - (1 - confidence) / 2) * 100)
    ...
    def test_mean(self, null_value: float = 0, alternative: str = 'two-sided',
                 reps: int = 999, confidence: float = 0.95) -> HypothesisTestResult:
    ...
    def test_median(self, null_value: float = 0, alternative: str = 'two-sided',
                   reps: int = 999, confidence: float = 0.95) -> HypothesisTestResult:
    ...
    def test_variance(self, null_value: float = 1, alternative: str = 'two-sided',
                     reps: int = 999, confidence: float = 0.95) -> HypothesisTestResult:
    ...
    def test_correlation(self, y: np.ndarray, null_value: float = 0,
                        alternative: str = 'two-sided', reps: int = 999,
                        confidence: float = 0.95) -> HypothesisTestResult:
    ...
    def compare_means(self, y: np.ndarray, null_value: float = 0,
                     alternative: str = 'two-sided', reps: int = 999,
                     confidence: float = 0.95) -> HypothesisTestResult:
